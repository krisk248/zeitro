<script lang="ts">
  ...
  import { register, isLoading, user } from '$lib/auth';
  ...
  <div class="w-full max-w-[340px]">
    ...
    <form onsubmit={handleSubmit} class="flex flex-col gap-4">
      <div class="flex flex-col gap-1.5">
        <label for="displayName" class="text-[11px] font-medium uppercase tracking-widest text-muted-foreground">
          Display name
        </label>
        <input
          id="displayName"
    ...
    <p class="mt-6 text-center text-[12px] text-muted-foreground">
      ...
      <a href="/login" class="text-foreground underline underline-offset-2 hover:opacity-70 transition-opacity">
        Sign in
      </a>
    </p>
  </div>
